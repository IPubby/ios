...
NSString * NSStringFromArticleType(type)
{
    switch (type) {
        case ArticleTypeMostViewed:
            return ArticleProviderResourceTypeMostViewed;
        case ArticleTypeMostShared:
            return ArticleProviderResourceTypeMostShared;
        case ArticleTypeMostEmailed:
            return ArticleProviderResourceTypeMostEmailed;
        default:
            return nil;
    }
}
...
- (instancetype)initWithArticleType:(ArticleType)type
{
    self = [super init];
    if (self)
    {
...
- (id)initWithCoder:(NSCoder *)aDecoder
{
    ...
    if (self)
    {
        ...
        _numberOfArticles = [aDecoder decodeIntegerForKey:@"numberOfArticles"];
        _loading = NO;
        _offset = [aDecoder decodeIntegerForKey:@"offset"];
        _articleType = [aDecoder decodeIntegerForKey:@"articleType"];
    }
...
- (void)encodeWithCoder:(NSCoder *)aCoder
{
    ...
    [aCoder encodeInteger:self.numberOfArticles forKey:@"numberOfArticles"];
    [aCoder encodeInteger:self.offset forKey:@"offset"];
    [aCoder encodeInteger:self.articleType forKey:@"articleType"];
}
...
- (void)finishLoadingWithJSON:(NSDictionary *)json offset:(NSUInteger)offset
{
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        self.numberOfArticles = [numResults unsignedIntegerValue];
        ...
        {
            article.isFavorite = [[ArticlesController sharedInstance] isFavoriteArticle:article];
        }
...
- (void)loadArticlesForOffset:(NSUInteger)offset
{
    NSString *requestString = [ArticleProviderNYTimesBaseURI stringByAppendingFormat:
                               ...
                               NSStringFromArticleType(self.articleType),
...
{
    return self.numberOfArticles > self.articles.count;
}
...
- (void)loadMoreArticles
{
    if (!self.loading)
    {
        ...
        if (self.numberOfArticles == 0 || nextOffset < self.numberOfArticles)
        {
