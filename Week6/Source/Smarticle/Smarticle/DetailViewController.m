...
- (IBAction)didPressFavoriteButton:(id)sender
{
    if (self.article.isFavorite == YES)
    {
        ...
        self.favoriteButton.accessibilityLabel = @"Favorite off";
    }
    else
    {
        [[ArticlesController sharedInstance] addFavoriteArticle:self.article];
        [self.favoriteButton setTitle:@"★"];
        self.favoriteButton.accessibilityLabel = @"Favorite on";
    }
}
...
- (void)viewDidLoad
{
    ...
    self.favoriteButton.accessibilityHint = @"Toggles favorite";
    self.favoriteButton.accessibilityLabel = @"Favorite off";
